...
@interface WQOrderDealVC ()<UITableViewDelegate,UITableViewDataSource,WQCustomerOrderCellDelegate>
...
-(void)getOrderList {
    ...
    self.interfaceTask = [[WQAPIClient sharedClient] GET:@"/rest/order/orderList" parameters:@{@"lastOrderId":[NSNumber numberWithInteger:self.lastOrderId],@"count":[NSNumber numberWithInteger:self.limit],@"orderStatus":@"5"} success:^(NSURLSessionDataTask *task, id responseObject) {
