...
-(id)init {
    if (self = [super init]) {
        ...
        if (![rs next]) {
            [rs close];
            [self.db executeUpdate:@"CREATE TABLE WQUser (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE ,userId VARCHAR,userName VARCHAR,userHead VARCHAR,userPhone VARCHAR,password VARCHAR)"];
        }
        [rs close];
        rs = nil;
        
        //店铺
        FMResultSet *rs_store = [self.db executeQuery:@"select name from SQLITE_MASTER where name = 'WQStore'"];
        
        if (![rs_store next]) {
            [rs_store close];
            [self.db executeUpdate:@"CREATE TABLE WQStore (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE ,storeId VARCHAR,storeName VARCHAR,storeHead VARCHAR)"];
        }
        [rs_store close];
        rs_store = nil;
